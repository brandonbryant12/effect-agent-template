...
- [x] Add failing Postgres tests for CRUD, cross-user rejection, command idempotency, and event/job atomicity. Upload-token replay and concurrent queue claims remain in their owning later milestones.
...
- [x] Commit only this slice with `feat: add typed application persistence`.
